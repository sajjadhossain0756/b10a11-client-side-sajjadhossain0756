...
const Registration = () => {
  ...
  const handleSignUp = (e) => {
    ...
    const reguExp = /^(?=.*[A-Z])(?=.*[a-z])[A-Za-z0-9]{6,}$/;

    if(!reguExp.test(password)){
      Swal.fire({
        icon: "error",
        title: "Sorry!...",
        text: "Password Should be One Uppercase,One Lowercase and length will be 6 characters",
      });
      return;
    }
    ...
    createUser(email, password)
      .then(result => {
        ...
        .then(()=>{
          Swal.fire('Congratulaton! Your Registration Successful')
          navigate('/')
        })
        .catch(()=>{
